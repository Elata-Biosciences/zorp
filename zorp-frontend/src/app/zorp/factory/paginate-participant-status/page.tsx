...
export default function ZorpFactoryReadPaginateSubmittedData() {
	...
	const { data: statuses, isFetching, refetch } = useReadContract<
		typeof IZorpFactory.abi,
		'paginateParticipantStatus',
	...
	return (
		<div className="w-full flex flex-col">
			<h1 className="flex flex-col sm:flex-row justify-center items-center text-4xl font-bold">
				Zorp Factory -- Paginate Study Participant Statuses
			</h1>
			...
			<button
				...
				disabled={isFetching}
			>Get Study Participant Statuses</button>

			<section>
				<header>Study Participant Statuses</header>
				<ul>
					{ (statuses as number[])?.map((status, i) => {
						return (
							<li key={`${i}-${status}`}>
								{i + start} -- {status.toString()}
							</li>
						);
					}) }
				</ul>
